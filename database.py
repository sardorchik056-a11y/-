...
PANDA_COLUMNS = {
    ...
    # Роса и волшебный орех — редкие предметы-заготовки под будущую
    # прокачку уровня панды (наравне с wonder_bamboo выше), добываются
    # только кликами по дереву. Расходовать их пока негде — копятся
    # в инвентаре.
    "wonder_dew": "INTEGER NOT NULL DEFAULT 0",
    "magic_nut": "INTEGER NOT NULL DEFAULT 0",
    # Карма — пока просто копящаяся заглушка без применения в игре,
    # начисляется кликами по дереву, когда не выпало ни одного из
    # редких предметов выше. Что с ней делать дальше — не решено.
