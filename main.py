...
settings = {
    ...
    "rules": (
        '<b>1. Оплата 💬</b>\n\n'
        '• <blockquote>Номер должен простоять <b>5 минут</b>\n'
        '• Вы получаете <b>4.5 $</b> за один сеанс аренды\n'
        '• Если номер слетел до 5 минут — оплата <b>не начисляется</b></blockquote>\n\n'
        '<b>2. Обязанности сдатчика 💬</b>\n\n'
        '• <b><blockquote>Нельзя отвязывать номер</b>\n'
        '• <b>Отвяз</b> — штраф 15 $ (списывается с вашего баланса)\n'
        '• Нельзя сдавать один и тот же номер через несколько раз</blockquote>\n\n'
        '<b>3. Очередь 💬</b>\n\n'
        '• <blockquote>Номера принимаются <b>строго по очереди</b>\n'
        '• Если ваш номер сейчас используется — ждите, пока освободится</blockquote>'
    ),
}
